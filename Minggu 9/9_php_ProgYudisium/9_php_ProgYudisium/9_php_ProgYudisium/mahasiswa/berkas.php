                        ...
                        <div class="block">
                            ...
                            <div class="block-content collapse in">
                                <div class="span12">
										...
										<div class="controls">
										<select name="berkass" id="select01" class="span6 chzn-select" required>
...
<?php
	  ...
	  @$berkas2=mysql_fetch_array(mysql_query("select count(id_jenis) from jenis_berkas where id_jenis not in ( select id_jenis from detail_berkas where id_pendaftaran='$pendf');"));
          ...
          if($eror=="true"){
            ...
            if(move_uploaded_file($file_tmp, $folder.$newfilename)){
             ...
             $catat = mysql_query("INSERT INTO detail_berkas
                VALUES ('$berkass','$pendf','$newfilename','0')");
				if ( $berkas2 == $berkas ) {
					$catat2 = mysql_query("UPDATE `pendaftaran`.`pendaftaran` SET `STATUS_MHS` = '0' WHERE `pendaftaran`.`ID_PENDAFTARAN` = '$pendf';");
				} else {
					$catat2 = mysql_query("UPDATE `pendaftaran`.`pendaftaran` SET `STATUS_MHS` = '0' WHERE `pendaftaran`.`ID_PENDAFTARAN` = '$pendf';");
				}
      ...
      ?>		<fieldset></fieldset>
	  									<fieldset>
                                        <legend align="center">Daftar Berkas Persyaratan Yudisium</legend>
										<?php
										
										?>
										Sisa File yang belum diupload : <?php if ($dataa[0]==0){
											echo "-";
										} else { 
										echo $dataa[0];
										}										?>
                                ...
                                <div class="span12">
						              ...
						              <?php
									  $no=1;
$str=mysql_query("SELECT j.nama_jenis, db.status_validasi, j.id_jenis, db.FILE
...
?>
<td><center><?php echo $message;?>
<a href="uploadulang.php?uploadulang=<?php echo @$data[2]; ?>" class="btn btn-warning btn-mini <?php echo $ok; ?>">Upload Ulang</a>
<a href="../uploadberkas/<?php echo @$data[3]; ?>" class="btn btn-info btn-mini <?php echo $ok; ?>" target="_blank" >Lihat</a>
</center>
</td>
<?php echo "<td>";
 if ($data[1]=='0') {
	echo '<center><i class="icon-remove"><i></center>';
} else if ($data[1]=='1') {
	echo '<center><i class="icon-check"><i></center>';
} else {
	echo "Tidak Valid";
} echo"</td>"; ?>
<?php echo "</tr>" ;
	  ...
	  <?php } ?>
							...
							<div class="navbar navbar-inner block-header">
								<div class="row-fluid padd-bottom" align="center">
									...
									<?php 
							$muncul = mysql_query("SELECT count(db.status_validasi) FROM detail_berkas db, jenis_berkas j WHERE J.ID_JENIS=DB.ID_JENIS AND ID_PENDAFTARAN='$pendf' AND db.status_validasi='1'");
							$muncul1=mysql_fetch_array($muncul);
							//var_dump($muncul1);
							?>
	                                		<?php if ($muncul1[0]==$dataaa[0]){ ?>
												<a href="syarat-kelulusan-cetak.php" target="_blank" class="btn btn-info tooltip-top" data-original-title="Cetak Syarat Kelulusan" >Cetak</a>;
											<?php } 
											else { ?>
											<a class="btn btn-danger tooltip-top" data-original-title="Anda Belum Bisa Cetak" disabled="true">Cetak</a>
											<?php } ?>
									</div>
								</div>
							</div>
