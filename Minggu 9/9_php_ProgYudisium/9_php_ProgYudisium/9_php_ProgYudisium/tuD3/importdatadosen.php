...
@$getID = mysql_fetch_array(mysql_query("SELECT p.ID_PRODI from fakultas f, departemen d, program_studi p where f.ID_FAKULTAS=d.ID_FAKULTAS AND d.ID_DEPARTEMEN=p.ID_DEPARTEMEN AND p.nama_prodi='$userMobile1'"));
@$getIDkota = mysql_fetch_array(mysql_query("SELECT id_kota from kota where nama_kota='$namakota'"));
$query = "SELECT nip FROM dosen WHERE nip = '".$userName."' and nama_dosen = '".$userMobile."'";
$jk = trim($allDataInSheet[$i]["F"]);
$alamat = trim($allDataInSheet[$i]["G"]);
$tgl_lahir = trim($allDataInSheet[$i]["E"]);
$email = trim($allDataInSheet[$i]["H"]);
$jabatan = trim($allDataInSheet[$i]["I"]);
@$getIDjab = mysql_fetch_array(mysql_query("SELECT id_jabatan from jabatan where nama_jabatan='$jabatan'"));

$notelp = trim($allDataInSheet[$i]["J"]);
...
$insertdosen = mysql_query("INSERT INTO dosen (NIP, ID_KOTA, ID_PRODI, ID_JABATAN, PASSWORD, NAMA_DOSEN, TGL_LAHIR_DOSEN, ALAMAT, NO_TELP, JK, EMAIL) VALUES ('".$userName."','".$getIDkota[0]."','".$getID[0]."','".$getIDjab[0]."','".$pass."','".$userMobile."','".$format."','".$alamat."','".$notelp."','".$jk."','".$email."');");
//$insertTable= mysql_query("insert into mahasiswa (nim, nama, id_bidangilmu, id_kota, password, jenis_kelamin, agama, tgl_lahir ) values('".$userName."', '".$userMobile."','".$getID[0]."','".$getIDkota[0]."','".$pass."','".$jk."','".$agama."','".$format."');");			
	...
	//echo "<script>alert('Data Sudah Ada!!!!!!!!');</script>";
				$pesan1 ="2";
				echo("<META HTTP-EQUIV=\"Refresh\"CONTENT=\"0;URL=datamaster-dosen.php?pesan=$pesan1\">");
...
$pesan2="1";
				echo("<META HTTP-EQUIV=\"Refresh\"CONTENT=\"0;URL=datamaster-dosen.php?pesan=$pesan2\">");
