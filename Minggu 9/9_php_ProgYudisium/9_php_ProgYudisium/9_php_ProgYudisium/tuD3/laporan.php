...
@$d = mysql_fetch_array(mysql_query("SELECT ID_PEG, NAMA_PEG from tata_usaha where ID_PEG='$idpeg'"));
                            ...
                            <div class="block-content collapse in">
                                ...
                                <div class="span12">
                                     <form class="form-horizontal" method="POST" action="" enctype="multipart/form-data">
                                      <fieldset>
										...
										$sql = mysql_query("select year(p.BATAS_AKHIR) as tahun from detail_pelaksanaan dp, pelaksanaan_yudisium p, periode per where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN AND p.id_periode=per.id_periode group by year(p.BATAS_AKHIR) desc") or die (mysql_error());
										...
										<div class="control-group">
										<label class="control-label" for="select02" > Format Laporan</label>
										<div class="controls">
										<select name="format" id="select02" class="span6 chzn-select" required>
										<option value=""> Pilih Format</option>
										<option value="1"> Berdasarkan Format dari Fakultas</option>
										<option value="2"> Berdasarkan IPK</option>
                                ...
                                <div class="span12">
                                   <div class="table-toolbar">
                                    ...
                                    <legend align="center">Daftar Mahasiswa Lulus Yudisium</legend>
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2">
									  ...
									  $no=1;
									  if ($format==1){
$str=mysql_query("select m.NIM, m.NAMA, k.NAMA_KOTA, m.TGL_LAHIR, p.BATAS_AKHIR, ps.NAMA_PRODI, per.nama_periode, PE.IPK   
...
where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN and dp.ID_PENDAFTARAN=pe.ID_PENDAFTARAN and f.ID_FAKULTAS=d.ID_FAKULTAS AND d.ID_DEPARTEMEN=ps.ID_DEPARTEMEN AND b.ID_PRODI=ps.ID_PRODI AND b.ID_BIDANGILMU=m.ID_BIDANGILMU AND p.id_periode=per.id_periode and year(p.BATAS_AKHIR)='$tahun' and pe.NIM=m.NIM and per.nama_periode='$periode' and m.ID_KOTA=k.ID_KOTA and dp.STATUS_LULUS='1' order by m.NIM asc"); }
else if ($format==2) {
	$str=mysql_query("select m.NIM, m.NAMA, k.NAMA_KOTA, m.TGL_LAHIR, p.BATAS_AKHIR, ps.NAMA_PRODI, per.nama_periode, PE.IPK   
from detail_pelaksanaan dp, pelaksanaan_yudisium p, pendaftaran pe, mahasiswa m, kota k, fakultas f, departemen d, program_studi ps, bidangilmu b, periode per
where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN and dp.ID_PENDAFTARAN=pe.ID_PENDAFTARAN and f.ID_FAKULTAS=d.ID_FAKULTAS AND d.ID_DEPARTEMEN=ps.ID_DEPARTEMEN AND b.ID_PRODI=ps.ID_PRODI AND b.ID_BIDANGILMU=m.ID_BIDANGILMU AND p.id_periode=per.id_periode and year(p.BATAS_AKHIR)='$tahun' and pe.NIM=m.NIM and per.nama_periode='$periode' and m.ID_KOTA=k.ID_KOTA and dp.STATUS_LULUS='1' order by PE.IPK desc");
}
...
echo
"<tr><td>".$no.".</td><td>".$data[0]."</td><td>".$data[1]."</td><td>".$data[2].", ".$date1."</td><td>".$date2."</td><td>".$data[7]."</td>" ?>
...
?>
							...
							<div class="navbar navbar-inner block-header">
								<div class="row-fluid padd-bottom" align="center">
									<div style="padding:1px;">
	                                		<a href="laporan-yudisium-cetak.php?tahun=<?php echo $tahun; ?>&periode=<?php echo $periode; ?>&format=<?php echo $format; ?>" class="btn btn-primary tooltip-top" target="_blank" data-original-title="Cetak Laporan Mahasiswa Lulus Yudisium" >Cetak</a>
									</div>
								</div>
							</div>
                        </div>
