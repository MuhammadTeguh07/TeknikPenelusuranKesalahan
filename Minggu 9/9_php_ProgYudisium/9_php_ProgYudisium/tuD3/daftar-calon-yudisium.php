					...
					<div class="row-fluid">
                        ...
                        <div class="block" >
                            <div class="navbar navbar-inner block-header" >
                                <div class="muted pull-left" ><strong>Daftar Calon Mahasiswa Yudisium</strong></div>    
                            </div>
                            <div class="block-content collapse in">
                                ...
                                <div class="span12">
                                    ...
                                    <legend align="center">Daftar Calon Mahasiswa Yudisium</legend>
									<form class="form-horizontal" method="POST" action="" enctype="multipart/form-data">
									<div class="control-group">
										<label class="control-label" for="select02" > Tanggal Yudisium</label>
										<div class="controls">
										<select name="periode" id="select02" class=" chzn-select" required>
										<option value="">Pilih Tanggal Yudisium</option>
										<option value="1">Tampilkan Semua</option>
										<?php  
				
										$sql = mysql_query("select p.BATAS_AKHIR from detail_pelaksanaan dp, pelaksanaan_yudisium p, periode per where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN AND p.id_periode=per.id_periode group by p.BATAS_AKHIR desc") or die (mysql_error());
			
										while ($array=mysql_fetch_array($sql))
										{ 
										@$date = date('d-m-Y', strtotime($array[0]));
										?>
										...
										<?php 
											...
											?>
									Tanggal Yudisium : <?php
										if($periode==1){
											echo "Tampilkan Semua";
										} else {
											echo $periode;
										}
									?> <br><br>
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2">
										...
										<tbody>
                                         <?php
									  $no=1;
									  @$date5 = date('Y-m-d', strtotime($periode));
									  if ($periode==1){
$str=mysql_query("select m.nim, m.nama, k.NAMA_KOTA, m.TGL_LAHIR, m.NO_HP, dp.TGL_KEPUTUSAN_YUDISIUM, ps.NAMA_PRODI, pe.TOTAL_SKS, pe.IPK, pe.SKOR_ELPT, pe.NILAI_SKP, pe.LULUS_PPKMB, pe.PRESENTASE_NILAI_D, p.BATAS_AKHIR
from detail_berkas db, detail_pelaksanaan dp, pelaksanaan_yudisium p, pendaftaran pe, mahasiswa m, kota k, fakultas f, departemen d, program_studi ps, bidangilmu b, jenis_berkas jb
where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN and dp.ID_PENDAFTARAN=pe.ID_PENDAFTARAN and f.ID_FAKULTAS=d.ID_FAKULTAS AND d.ID_DEPARTEMEN=ps.ID_DEPARTEMEN AND b.ID_PRODI=ps.ID_PRODI AND b.ID_BIDANGILMU=m.ID_BIDANGILMU and db.id_pendaftaran=pe.id_pendaftaran and pe.status_mhs='1' and pe.ID_PENDAFTARAN=dp.ID_PENDAFTARAN and pe.nim=m.nim and dp.STATUS_LULUS='0'  group by m.nim order by m.nim asc");  
									  } else {
$str=mysql_query("select m.nim, m.nama, k.NAMA_KOTA, m.TGL_LAHIR, m.NO_HP, dp.TGL_KEPUTUSAN_YUDISIUM, ps.NAMA_PRODI, pe.TOTAL_SKS, pe.IPK, pe.SKOR_ELPT, pe.NILAI_SKP, pe.LULUS_PPKMB, pe.PRESENTASE_NILAI_D, p.BATAS_AKHIR
from detail_berkas db, detail_pelaksanaan dp, pelaksanaan_yudisium p, pendaftaran pe, mahasiswa m, kota k, fakultas f, departemen d, program_studi ps, bidangilmu b, jenis_berkas jb
where dp.ID_PELAKSANAAN=p.ID_PELAKSANAAN and dp.ID_PENDAFTARAN=pe.ID_PENDAFTARAN and f.ID_FAKULTAS=d.ID_FAKULTAS AND d.ID_DEPARTEMEN=ps.ID_DEPARTEMEN AND b.ID_PRODI=ps.ID_PRODI AND b.ID_BIDANGILMU=m.ID_BIDANGILMU and db.id_pendaftaran=pe.id_pendaftaran and pe.status_mhs='1' and pe.ID_PENDAFTARAN=dp.ID_PENDAFTARAN and pe.nim=m.nim and dp.STATUS_LULUS='0' and p.BATAS_AKHIR='$date5' group by m.nim order by m.nim asc");
									  }
				
while($data=mysql_fetch_array($str))
{
				@$date1 = date('d-m-Y', strtotime($data[3]));
				@$date2 = date('d-m-Y', strtotime($data[5]));
				@$date3 = date('d-m-Y', strtotime($data[13]));
echo
"<tr><td>".$no.".</td><td>".$data[0]."</td><td>".$data[1]."</td><td>".$data[2].", ".$date1."</td><td>".$data[4]."</td><td>".$date3."</td><td>".$data[8]."</td>";
if($data[7]=='1'){
	$ok='hidden';
	$message="Lulus Yudisium";
}else if($data[7]=='2'){
	$ok='hidden';
	$message="Belum Lulus";
} else {
...
?>
<td><a href="daftar-berkas-mahasiswa.php?nim=<?php echo $data[0]; ?>" class="btn btn-info btn-mini" >Detail</a></td>
<?php echo "</tr>" ;
$no++;
}
...
?>
							...
							<div class="navbar navbar-inner block-header">
								<div class="row-fluid padd-bottom" align="center">
									<div style="padding:1px;">
	                                		<a href="daftar-yudisium-cetak.php?periode=<?php echo $periode; ?>" class="btn btn-primary tooltip-top" target="_blank" data-original-title="Cetak Daftar Calon Mahasiswa Yudisium" >Cetak</a>
									</div>
								</div>
							</div>
                        </div>
