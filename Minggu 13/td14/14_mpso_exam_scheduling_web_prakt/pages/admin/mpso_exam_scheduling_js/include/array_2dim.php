<?php
/*
	mengambil data dari tabel database mysqli ==> array 2 dimensi (matriks)
*/

function ambil_data_tabel($dbhandle, $nama_tabel){
	//echo "masuk ambil_data_tabel </br>"; 
	//echo "koneksi".$mydb."</br>";
	//echo "nama_tabel : ".$nama_tabel."</br>";   
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel"); //, MYSQLI_USE_RESULT);
...
function ambil_data_tabel_urut($dbhandle,$nama_tabel, $urut){
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel ORDER BY '$urut' ASC");
	$matNama_Tabel_php = array();
	$nNama_Tabel_php = 0;
	$n_colR = 0;
	while($row = mysqli_fetch_row($baca_nama_tabel)){
...
*/
function ambil_data_slotisi($dbhandle,$nama_tabel,$kd_thn_ajaran,$kd_semester ,$kd_masa_ujian){
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel"
...
*/
function delete_slotisi($dbhandle,$nama_tabel, $kd_thn_ajaran, $kd_semester ,$kd_masa_ujian){
	//echo "nama_tabel : ".$nama_tabel." <br>";
	//echo "kd_thn_ajaran : ".$kd_thn_ajaran." <br>";
	//echo "kd_semester : ".$kd_semester." <br>";
	//echo "kd_masa_ujian : ".$kd_masa_ujian." <br>";
			
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel WHERE `KODETHNAJARAN`= '$kd_thn_ajaran' AND `KODESEMESTER` = '$kd_semester' AND `masaujian` = $kd_masa_ujian ORDER BY `nilaifitness` DESC");
	
		if (!$baca_nama_tabel) {
			echo 'Could not run query: ' . mysqli_error($dbhandle);
			exit;
			}
			
	//nilaifitness
	//$baca_nama_tabel = mysqli_query("SELECT * FROM $nama_tabel"
	//."WHERE `KODETHNAJARAN`= '$kd_thn_ajaran' AND `KODESEMESTER` = '$kd_semester' AND `masaujian` = $kd_masa_ujian"
	...
	if($nNama_Tabel_php != 0) {
		...
		$result = mysqli_query($dbhandle,"DELETE FROM $nama_tabel WHERE `nilaifitness` = '$nil_maks' "
		."AND `ket` = '$nil_ket' ");
		}
...
*/
function ambil_detailkelas($dbhandle,$nama_tabel, $nama_tabel2 ,$kd_thn_ajaran, $kel){
	//echo "nama_tabel : ".$nama_tabel." <br>";
	//echo "nama_tabel2 : ".$nama_tabel2." <br>";
	//echo "kd_thn_ajaran : ".$kd_thn_ajaran." <br>";
	//echo "kel : ".$kel." <br>";

	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT dk.`KODEPRODI`,dk.`KODEKELAS`,dk.`kdpecah`,dk.`KODEMATKUL`, "
...
*/
function ambil_labkomujian($dbhandle,$nama_tabel, $nama_tabel2 ,$kd_thn_ajaran, $kd_semester){
	//echo "nama_tabel : ".$nama_tabel." <br>";
	//echo "nama_tabel2 : ".$nama_tabel2." <br>";
	//echo "kd_thn_ajaran : ".$kd_thn_ajaran." <br>";
	//echo "kel : ".$kel." <br>";

	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT nt.`KODELABUJIAN`,nt.`KODELAB`, nt2.`KAPASITASLAB`, nt2.`NAMALAB`"
...
*/
function ambil_ruangujian($dbhandle,$nama_tabel, $nama_tabel2 ,$kd_thn_ajaran, $kd_semester){
    //echo "nama_tabel : ".$nama_tabel." <br>";
	//echo "nama_tabel2 : ".$nama_tabel2." <br>";
	//echo "kd_thn_ajaran : ".$kd_thn_ajaran." <br>";
	//echo "kd_semester : ".$kd_semester." <br>";
	
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel AS nt "
	."WHERE nt.`KODETHNAJARAN`= '$kd_thn_ajaran' AND nt.`KODESEMESTER`='$kd_semester' ");
	...
	for($i=0;$i<$nNama_Tabel_php;$i++){
		if($matNama_Tabel_php[$i][2] == null){		
			...
			mysqli_query("UPDATE `ruangujian` SET `koderuang2` = '-' "
				."WHERE `ruangujian`.`KODERU` = '$indeks'");
		}
	}
//echo "============= matNama_Tabel_php : <br>";
//tampilMatriks($matNama_Tabel_php);echo "<br>";	
	$baca_nama_tabel2 = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel2");	
...
//echo "============= has : ".$has." dan nil : ".$nil." <br>";		
		...
		$indeks = $matNama_Tabel_php[$i][0];
		mysqli_query($dbhandle,"UPDATE `ruangujian` SET `kapasitas` = '$has' "
		."WHERE `ruangujian`.`KODERU` = '$indeks'");	
...
*/
function cek_hari_jumat($dbhandle,$nama_tabel){
	//echo "nama_tabel : ".$nama_tabel." <br>";
	//$nj = array();	
	//if (!$baca_nama_tabel) {
	...
	$hit = 0;
	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT KODEHARI,NAMAHARI FROM hari ");
	while($row = mysqli_fetch_row($baca_nama_tabel)) { 
	...
	if($hit > 0) {
		...
		if($nama_tabel == "hari") 
			{
			$baca_nama_tabel_h = mysqli_query($dbhandle,"SELECT * FROM $nama_tabel WHERE NAMAHARI='jumat'");
			$row = mysqli_fetch_row($baca_nama_tabel_h);
			//echo "hari_jumat : ".$row[1]." <br>";
			}
		if($nama_tabel == "hariujian") 
			{
			$baca_nama_tabel_hu = mysqli_query($dbhandle,"SELECT hu.`KODEHARIUJIAN`,hu.`KODEHARI`, h.`NAMAHARI`,hu.`KODESEMESTER` ,hu.`KODETHNAJARAN`"
...
*/
function ambil_peg_pengawas($dbhandle,$nama_tabel,$kd_fakultas, $kd_jabatan){
	//echo "nama_tabel : ".$nama_tabel." <br>";
	//echo "nama_tabel2 : ".$nama_tabel2." <br>";
	//echo "kd_thn_ajaran : ".$kd_thn_ajaran." <br>";
	//echo "kel : ".$kel." <br>";

	$baca_nama_tabel = mysqli_query($dbhandle,"SELECT p.`NIP`,p.`NAMAPEGAWAI` FROM $nama_tabel AS p "
